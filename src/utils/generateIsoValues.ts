 ...
 * @category Utils
 */
 export function generateIsoValues(min: number, max: number, nbrOrSpacingOrList: number | number[] = 10, useSpacing=false): Array<number> {
    if (Array.isArray(nbrOrSpacingOrList)) {
        return nbrOrSpacingOrList.filter( v => v>=min && v<=max)
    }
    if (useSpacing) {
        return generateIsoValuesBySpacing(min, max, nbrOrSpacingOrList as number)
    }
    return generateIsoValuesByNumber(min, max, nbrOrSpacingOrList as number)
}
 ...
 * @category Utils
 */
export function generateIsoValuesByNumber(min: number, max: number, nbr: number = 10): Array<number> {
    if (min >= max) {
        return []
    }
    const epsilon = ( max - min) / (nbr)
    return generateIsoValuesBySpacing(min, max, epsilon)
}
 ...
 * @category Utils
 */
export function generateIsoValuesBySpacing(min: number, max: number, spacing: number): Array<number> {
    const r = []
